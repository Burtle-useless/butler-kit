...
# 錯誤樣態不准當標題——另一個前端曾把「Failed to authenticate. API Error: 401」
...
async def generate_title(convo_text: str) -> str | None:
    ...
    這種沒有資訊量的話，只讀它就只能生出同等級的標題。3000 字對齊另一個前端
    讀 session 內容的預設值——夠涵蓋前幾輪往返，又不會讓 Haiku 讀太久。
    """
    try:
