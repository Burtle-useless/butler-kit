...
助理會很有自信地說「你現在在辦公室」，而使用者三小時前就離開了。所以這裡花最多
篇幅釘住的是「退回快取時一定要標明幾分鐘前」。

另一條是別重複吵手機：每呼叫一次工具就是使用者的手機開一次 GPS，而那邊不會看到
...
def test_max_age_zero_forces_refetch() -> None:
    ...
    with patch.object(location, "LOCATION_FILE", f), \
        ...
        # 快取要在 patch 還生效時讀，不然讀到的是本機真正的 location.json
