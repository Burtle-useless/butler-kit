...
def force_confirm_on() -> None:
    ...
    總開關是 `CONFIRM_DANGEROUS` 這個環境變數（`config.CONFIRM_ENABLED` 讀它）：
    設 0 全放行、設 1 才會啟用確認。啟動腳本可以把它寫死，測試在那種環境底下
    跑就會繼承那個值。
...
def test_fail_closed() -> None:
    ...
    #
    # 把後果講明白：`CONFIRM_DANGEROUS=0` 的部署底下，破壞性指令不會跳確認，
    # 直接就跑了。要不要關是部署時自己的取捨，這裡不替它決定，
    # 只把後果釘在測試裡讓它是明講的
    try:
