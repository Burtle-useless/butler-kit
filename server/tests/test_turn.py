...
async def test_auto_continue() -> None:
    ...
    # 而它早就逐字打在畫面上了，中間那段空窗看起來就是「打了又收回去」。
...
async def test_ask_inline() -> None:
    ...
    舊版是伺服器停在 frontend.ask() 等答案、五分鐘沒回就把卡片收掉。手機上的
    回覆常常隔更久才來，回來只看到選項不見了、也不知道剛才被問了什麼。
    2026-08-19 改成選項是訊息的一部分，什麼時候點都算數。
    """
    print("\n[提問不阻塞]")
    ask_data = {"questions": [{
        "question": "要刪哪一個？",
...
async def test_ask_marker() -> None:
    ...
    從頭到尾一次都沒觸發過——連續好幾輪「選項面板出不來」就是這個原因。
...
async def test_compact_is_silent() -> None:
    ...
    先前它跟正常回覆走同一條路串流出去，畫面上會看到一大段對話摘要被打出來、
    然後被下一個 turn.start 清空——症狀就是「顯示一堆對話摘要再收回去」。
    """
    print("\n[壓縮不外洩]")
    s = Scripted([
        TurnResult(reply="摘要好了", session_id="new-sid"),
...
async def test_recheck_after_compact() -> None:
    ...
    重啟後的摘要卻寫著「已經說過記好了」——帳真的進去了，使用者卻從頭到尾
    沒看到任何確認，只能自己發現提過的東西後面不見了。
...
async def test_turn_done_timing() -> None:
    ...
    症狀：收到「做完了」的推播，點進來助理還在思考。原因是推播綁在
    `reply.final` 上，而那個事件**每一輪都會發**——自動續跑每續一輪一則、
    壓縮核對再一則。通知只應該在整件事收尾時發那一次。
    """
    print("\n[收工通知的時機]")
    s = Scripted([
