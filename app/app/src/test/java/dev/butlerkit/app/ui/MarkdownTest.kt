 ...
 * 行內連結的切割。
 *
 * 在聊天裡要能直接點網址，不要複製貼上。能點與不能點的差別只在
...
class MarkdownTest {
    ...
    fun `裸網址可以點，句末的句號不算在網址裡`() {
        assertEquals(
            listOf("https://example.com/docs" to "https://example.com/docs"),
            links("設定在 https://example.com/docs。點進去登入就好"),
        )
    }
    ...
    fun `markdown 連結顯示標題、點進去是網址`() {
        assertEquals(
            listOf("說明文件" to "https://example.com/"),
            links("[說明文件](https://example.com/) 要先登入"),
        )
        // 標題那段字本身不能留下方括號
        assertTrue(inline("[說明](https://a.b)").text == "說明")
    }
